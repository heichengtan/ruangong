...
public class CommentDAO {
	public ArrayList<CommentBean> getComment(String ISO3166) {
		...
		try {
			...
			String query = "SELECT content,comment_time,comment.member_num,comment_seq,nickname FROM comment " + 
					"INNER JOIN member on comment.member_num = member.member_num " + 
					"WHERE ISO3166 = ?";
			...
			while (rs.next()) // Until next row is present otherwise it return false
			{
				...
				commentBean.setNickname(rs.getString(5));
