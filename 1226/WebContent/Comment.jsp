...
<script>
	function validate_insert() {
		...
		if (insert == null || insert == "") {
			...
			document.getElementById("insert_label").innerText="最多輸入兩百個字元"; 
			document.getElementById("insert_label").style.display = 'block';
			return false;
		}
		else{
			document.getElementById("insert_botton").dataset.target="#message";
			document.getElementById("insert_botton").dataset.toggle="modal";
			document.getElementById("insert_botton").click();
			document.getElementById("insert_botton").submit();
			return true;
		}
	}
	function validate_update() {
		...
		if (update == null || update == "") {
			...
			document.getElementById("update_label").innerText="最多輸入兩百個字元"; 
			document.getElementById("update_label").style.display = 'block';
			return false;
		}
		else{
			document.getElementById("update_botton").dataset.target="#message";
			document.getElementById("update_botton").dataset.toggle="modal";
			document.getElementById("update_botton").click();
			document.getElementById("update_botton").submit();
			return true;
		}
	}
</script>
...
<body>
	...
	<table class="table table-hover">
		<thead>
			<tr class="row">
				<th class="col-sm-4">留言內容</th>
				<th class="col-sm-2">留言時間</th>
				<th class="col-sm-2">留言者</th>
		...
		%>
		<tr class="row">
			<td class="col-sm-4"><%=commentbean.getContent()%></td>
			<td class="col-sm-2"><%=commentbean.getComment_time()%></td>
			<td class="col-sm-2"><%=commentbean.getNickname()%></td>
			<td class="col-sm-2">
			<%
						...
						<div class="modal-content">
					       ...
					       <div class="modal-body">
							<form action="UpdateCommentServlet" method="post"
								...
								<button class="btn btn-primary" id="update_botton">修改</button>
							</form>
							</div>
		...
		<div class="modal fade" id="newComment" >
			<div class="modal-dialog">
				<div class="modal-content">
			       ...
			       <div class="modal-body">
					<form action="NewCommentServlet" method="post" target="message2" onsubmit="return validate_insert()"  data-toggle="modal">
						...
						<button class="btn btn-primary" id="insert_botton" >新增</button>
					</form>
					</div>
					<div class="modal-footer">
